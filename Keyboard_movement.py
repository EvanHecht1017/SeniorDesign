...
def on_arrow_key(event):
    if event.name == 'w':
        move_x_axis(Move_Distance)
    elif event.name == 's':
        move_x_axis(-1 * Move_Distance)
    elif event.name == 'a':
        move_y_axis(Move_Distance)
    elif event.name == 'd':
        move_y_axis(-1 * Move_Distance)
    elif event.name == 'up':
        move_z_axis(-1 * Move_Distance)
    elif event.name == 'down':
        move_z_axis(Move_Distance)

# Hook the keyboard events
keyboard.on_press_key("up", on_arrow_key)
keyboard.on_press_key("down", on_arrow_key)
keyboard.on_press_key("a", on_arrow_key)
keyboard.on_press_key("s", on_arrow_key)
keyboard.on_press_key("d", on_arrow_key)
keyboard.on_press_key("w", on_arrow_key)

print("Control the gantry with arrow keys. Press CTRL+C to stop.")
try:
    while True:
        time.sleep(1)  # Main loop does nothing but sleep, key events are handled by callbacks.
except KeyboardInterrupt:
    print("User interrupted. Cleaning up...")

    # Assuming a function to close the device safely
    # hllApi_Close_Device(Username)
